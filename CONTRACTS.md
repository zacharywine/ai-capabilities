...
| `workflows/static-audit.md` | Severity groups in exact order: 1) `critical` → 2) `high` → 3) `medium` → 4) `low`; then a brief summary count by severity (heading optional) | Severity axis unchanged; if execution evidence is reported, include status `PASS`, `FAIL`, `BLOCKED`, or `UNKNOWN` with evidence | Yes |
